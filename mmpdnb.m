function gen_name=mmpdnb(expression_tumor_fileName,expression_normal_fileName,path,popnum,Max_CalNum,experiment_num)
%% *************************tumor data****************************
...
for fnum = 1:length(numf)-2 % all patient samples
    ...
    [PF,PS,Non_dominated_sol]=MMPDNB_EA(data,popnum,Max_CalNum,experiment_num);
    
    filename=strcat(path2,expression_tumor_fileName(1:4),'_sample_',num2str(fnum),'_MMPDNB_PF.mat');
    save(filename,'PF');
    filename2=strcat(path2,expression_tumor_fileName(1:4),'_sample_',num2str(fnum),'_MMPDNB_PS.mat');
    save(filename2,'PS');
    filename3=strcat(path2,expression_tumor_fileName(1:4),'_sample_',num2str(fnum),'_MMPDNB_boxchart.mat');
    save(filename3,'Non_dominated_sol');
end
%% output mutil-modal PDNB or PDNB of patient samples

for fnum=1:length(numf)-2 % all patient samples
    data=load([path1,expression_tumor_fileName(1:4),'_',num2str(fnum) '_PGIN']);
    Sol=load([path2,expression_tumor_fileName(1:4),'_sample_',num2str(fnum),'_MMPDNB_PF.mat']);
    Sol=Sol.PF;
    POP=load([path2,expression_tumor_fileName(1:4),'_sample_',num2str(fnum),'_MMPDNB_PS.mat']);
    POP=POP.PS;
    maxf=max(Sol(:,2));
    index=Sol(:,2)==maxf;
